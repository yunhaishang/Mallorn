    ...
    public class ExchangeRequestsRepository : Repository<ExchangeRequest>, IExchangeRequestsRepository
    {
        ...
        public async Task<IEnumerable<ExchangeRequest>> GetByOfferProductIdAsync(int productId)
        {
            return await _context.ExchangeRequests
                ...
                .Where(e => e.OfferProductId == productId)
        ...
        public async Task<IEnumerable<ExchangeRequest>> GetByRequestProductIdAsync(int productId)
        {
            return await _context.ExchangeRequests
                .Include(e => e.OfferProduct)
                .Include(e => e.RequestProduct)
                .Where(e => e.RequestProductId == productId)
                .OrderByDescending(e => e.CreatedAt)
                .ToListAsync();
        }
        ...
        public async Task<(IEnumerable<ExchangeRequest> Requests, int TotalCount)> GetPagedRequestsAsync(
        ...
        public async Task<IEnumerable<ExchangeRequest>> GetPendingExchangesAsync()
        {
            return await _context.ExchangeRequests
                .Include(e => e.OfferProduct)
                .Include(e => e.RequestProduct)
                .Where(e => e.Status == "等待回应")
                .OrderByDescending(e => e.CreatedAt)
                .ToListAsync();
        }

        public async Task<bool> AcceptExchangeAsync(int exchangeId)
        {
            return await UpdateExchangeStatusAsync(exchangeId, "接受");
        }

        public async Task<bool> RejectExchangeAsync(int exchangeId)
        {
            return await UpdateExchangeStatusAsync(exchangeId, "拒绝");
        }
        ...
        public async Task<IEnumerable<ExchangeRequest>> FindMatchingExchangesAsync(int productId)
        {
            ...
            return await _context.ExchangeRequests
                .Include(e => e.OfferProduct)
                .Include(e => e.RequestProduct)
                .Where(e => e.RequestProductId == productId && e.Status == "等待回应")
                .OrderByDescending(e => e.CreatedAt)
                .ToListAsync();
        }
        ...
        {
            // 查找可能的相互交换机会（A想要B的商品，B想要A的商品）
            var allRequests = await _context.ExchangeRequests
                .Include(e => e.OfferProduct)
                .Include(e => e.RequestProduct)
                .Where(e => e.Status == "等待回应")
                .ToListAsync();
            ...
            {
                var mutualRequest = allRequests.FirstOrDefault(r =>
                    r.ExchangeId != request.ExchangeId &&
                    r.OfferProductId == request.RequestProductId &&
                    r.RequestProductId == request.OfferProductId);
        ...
        {
            return await _context.ExchangeRequests
                .AnyAsync(e => (e.OfferProductId == productId || e.RequestProductId == productId) &&
                              e.Status == "等待回应");
        }
        ...
        public async Task<IEnumerable<dynamic>> GetPopularExchangeCategoriesAsync()
        {
            return await _context.ExchangeRequests
                ...
                .GroupBy(e => e.OfferProduct.Category.Name)
                .Select(g => new
                {
                    Category = g.Key,
                    Count = g.Count()
